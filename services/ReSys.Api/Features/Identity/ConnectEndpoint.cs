...
public class ConnectEndpoint : ICarterModule
{
    ...
    {
        app.MapPost("/api/connect/token", Exchange).DisableAntiforgery();
        app.MapGet("/api/connect/authorize", Authorize);
        app.MapPost("/api/connect/authorize", Authorize).DisableAntiforgery(); // Handle form post from login
    }
