  ...
  <div class="container p-4 mx-auto md:p-8">
    ...
    <div v-if="loading" class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
      <div
        v-for="i in 8"
        :key="i"
        class="p-4 overflow-hidden border shadow-sm bg-surface-0 dark:bg-surface-900 rounded-4xl border-surface-100 dark:border-surface-800"
      >
    ...
    <div v-else class="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-4">
      <Card
        ...
        :style="example.hex_color ? { '--p-primary-color': example.hex_color } : {}"
      >
        <template #header>
          <div
            class="relative m-4 overflow-hidden shadow-inner aspect-4/5 rounded-4xl bg-surface-50 dark:bg-surface-800"
          >
            <img
            ...
            <div v-if="example.hex_color" class="absolute top-4 left-4">
              <div 
                class="w-8 h-8 border shadow-lg rounded-xl border-surface-0/20 backdrop-blur-md"
                :style="{ backgroundColor: example.hex_color }"
              ></div>
            </div>
