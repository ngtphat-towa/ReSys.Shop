...
namespace ReSys.Core.Features.Promotions.Admin.Rules.AddProductRule;

public static class AddProductRule
{
    public record Request(List<Guid> ProductIds, bool IsExclude = false);
    ...
    public class Handler(IApplicationDbContext context) : IRequestHandler<Command, ErrorOr<Success>>
    {
        public async Task<ErrorOr<Success>> Handle(Command command, CancellationToken ct)
        {
            ...
            var type = req.IsExclude ? PromotionRule.RuleType.ProductExclude : PromotionRule.RuleType.ProductInclude;

            var ruleResult = PromotionRule.Create(promotion.Id, type, new RuleParameters { TargetIds = req.ProductIds });
