...
namespace ReSys.Core.Features.Admin.Catalog.Taxonomies.GetTaxonomySelectList;

public static class GetTaxonomySelectList
{
    // Request:
    public record Request : QueryOptions;

    // Response:
    public record Response : TaxonomySelectListItem;
    ...
    public class Handler(IApplicationDbContext context) : IRequestHandler<Query, PagedList<Response>>
    {
        public async Task<PagedList<Response>> Handle(Query query, CancellationToken cancellationToken)
        {
            ...
            return await sortedQuery.ToPagedOrAllAsync<Taxonomy, Response>(
                request,
                cancellationToken);
        }
    }
