...
public class TaxonEvaluator : ITaxonEvaluator
{
    ...
    private Expression BuildNumericExpression(ParameterExpression parameter, TaxonRule rule)
    {
        ...
        var masterVariant = Expression.Call(typeof(Enumerable), "FirstOrDefault", [typeof(Domain.Catalog.Products.Variants.Variant)], 
            Expression.Property(parameter, nameof(Product.Variants)),
            Expression.Lambda<Func<Domain.Catalog.Products.Variants.Variant, bool>>(
                Expression.Property(Expression.Parameter(typeof(Domain.Catalog.Products.Variants.Variant), "v"), "IsMaster"),
                Expression.Parameter(typeof(Domain.Catalog.Products.Variants.Variant), "v")
            )
        );
