...
namespace ReSys.Core.Features.Admin.Catalog.Products.GetProductSelectList;

public static class GetProductSelectList
{
    // Request:
    public record Request : QueryOptions;

    // Response:
    public record Response : ProductSelectListItem;
    ...
    public class Handler(IApplicationDbContext context) : IRequestHandler<Query, PagedList<Response>>
    {
        public async Task<PagedList<Response>> Handle(Query query, CancellationToken cancellationToken)
        {
            ...
            return await sortedQuery.ToPagedOrAllAsync<Product, Response>(
                request,
                cancellationToken);
        }
    }
