...
public class UpdateExampleCategoryTests : BaseIntegrationTest
{
    ...
    public async Task Put_WithValidRequest_UpdatesCategory()
    {
        ...
        // Act
        var response = await AuthenticatedClient.PutAsJsonAsync($"/api/testing/example-categories/{id}", updateRequest, TestContext.Current.CancellationToken);
    ...
    public async Task Put_DuplicateName_ReturnsConflict()
    {
        ...
        await AuthenticatedClient.PostAsJsonAsync("/api/testing/example-categories", new CreateExampleCategory.Request { Name = name1 }, TestContext.Current.CancellationToken);
        var res2 = await AuthenticatedClient.PostAsJsonAsync("/api/testing/example-categories", new CreateExampleCategory.Request { Name = name2 }, TestContext.Current.CancellationToken);
        ...
        // Act
        var response = await AuthenticatedClient.PutAsJsonAsync($"/api/testing/example-categories/{cat2!.Data!.Id}", updateRequest, TestContext.Current.CancellationToken);
