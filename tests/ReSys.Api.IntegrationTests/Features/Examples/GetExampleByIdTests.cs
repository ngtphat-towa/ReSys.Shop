...
public class GetExampleByIdTests(IntegrationTestWebAppFactory factory) : BaseIntegrationTest(factory)
{
    ...
    public async Task GetById_ExistingExample_ReturnsExample()
    {
        ...
        var response = await Client.GetAsync($"/api/examples/{ExampleId}", TestContext.Current.CancellationToken);
    ...
    {
        var response = await Client.GetAsync($"/api/examples/{Guid.NewGuid()}", TestContext.Current.CancellationToken);
