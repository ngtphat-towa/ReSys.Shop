...
public class UpdatePropertyTypeTests : IClassFixture<TestDatabaseFixture>
{
    ...
    public async Task Handle_ValidRequest_ShouldUpdate()
    {
        ...
        var handler = new Core.Features.Admin.Catalog.PropertyTypes.UpdatePropertyType.UpdatePropertyType.Handler(_fixture.Context);
        var request = new Core.Features.Admin.Catalog.PropertyTypes.UpdatePropertyType.UpdatePropertyType.Request 
        { 
            Name = $"New_{Guid.NewGuid()}",
            Presentation = "Updated",
            Kind = PropertyKind.Integer
        };
        var command = new Core.Features.Admin.Catalog.PropertyTypes.UpdatePropertyType.UpdatePropertyType.Command(pt.Id, request);
    ...
    public async Task Handle_DuplicateName_ShouldReturnError()
    {
        ...
        var handler = new Core.Features.Admin.Catalog.PropertyTypes.UpdatePropertyType.UpdatePropertyType.Handler(_fixture.Context);
        var request = new Core.Features.Admin.Catalog.PropertyTypes.UpdatePropertyType.UpdatePropertyType.Request { Name = name2, Kind = PropertyKind.String };
        var command = new Core.Features.Admin.Catalog.PropertyTypes.UpdatePropertyType.UpdatePropertyType.Command(pt1.Id, request);
