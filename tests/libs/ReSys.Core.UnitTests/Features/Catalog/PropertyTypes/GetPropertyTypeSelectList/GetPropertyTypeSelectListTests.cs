...
public class GetPropertyTypeSelectListTests : IClassFixture<TestDatabaseFixture>
{
    ...
    public async Task Handle_NoPaging_ShouldReturnAll()
    {
        ...
        var handler = new ReSys.Core.Features.Catalog.PropertyTypes.GetPropertyTypeSelectList.GetPropertyTypeSelectList.Handler(_fixture.Context);
        var request = new ReSys.Core.Features.Catalog.PropertyTypes.GetPropertyTypeSelectList.GetPropertyTypeSelectList.Request 
        { 
            Filter = $"Name*{baseName}"
        };
        var query = new ReSys.Core.Features.Catalog.PropertyTypes.GetPropertyTypeSelectList.GetPropertyTypeSelectList.Query(request);
