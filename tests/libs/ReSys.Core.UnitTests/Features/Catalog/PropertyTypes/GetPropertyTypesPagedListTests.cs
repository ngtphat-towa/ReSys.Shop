...
public class GetPropertyTypesPagedListTests : IClassFixture<TestDatabaseFixture>
{
    ...
    public async Task Handle_ValidRequest_ShouldReturnPagedList()
    {
        ...
        var handler = new Core.Features.Admin.Catalog.PropertyTypes.GetPropertyTypesPagedList.GetPropertyTypesPagedList.Handler(_fixture.Context);
        var request = new Core.Features.Admin.Catalog.PropertyTypes.GetPropertyTypesPagedList.GetPropertyTypesPagedList.Request 
        { 
            Filter = $"Name*{baseName}",
            PageSize = 2
        };
        var query = new Core.Features.Admin.Catalog.PropertyTypes.GetPropertyTypesPagedList.GetPropertyTypesPagedList.Query(request);
