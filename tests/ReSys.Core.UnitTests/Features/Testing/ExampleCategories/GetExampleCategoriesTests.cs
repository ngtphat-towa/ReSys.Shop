...
public class GetExampleCategoriesTests : IClassFixture<TestDatabaseFixture>
{
    private readonly TestDatabaseFixture _fixture;

    public GetExampleCategoriesTests(TestDatabaseFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task Handle_WithFilter_ShouldReturnCorrectItems()
    {
        ...
        _fixture.Context.Set<ExampleCategory>().AddRange(
        ...
        var handler = new GetExampleCategories.Handler(_fixture.Context);
    ...
    public async Task Handle_NoMatches_ShouldReturnEmpty()
    {
        ...
        var result = await new GetExampleCategories.Handler(_fixture.Context).Handle(new GetExampleCategories.Query(request), TestContext.Current.CancellationToken);
    ...
    public async Task Handle_Pagination_Works()
    {
        ...
        for (int i = 1; i <= 5; i++)
        {
            _fixture.Context.Set<ExampleCategory>().Add(new ExampleCategory { Id = Guid.NewGuid(), Name = $"{baseName}_{i}" });
        }
        await _fixture.Context.SaveChangesAsync(TestContext.Current.CancellationToken);
