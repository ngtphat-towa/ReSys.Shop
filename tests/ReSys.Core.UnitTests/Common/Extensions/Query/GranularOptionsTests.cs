...
public class GranularOptionsTests : IClassFixture<TestDatabaseFixture>
{
    private readonly TestDatabaseFixture _fixture;

    public GranularOptionsTests(TestDatabaseFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task ApplyPagingAsync_WithProjection_Works()
    {
        ...
        _fixture.Context.Set<Example>().Add(new Example { Id = Guid.NewGuid(), Name = $"{baseName}_X" });
        await _fixture.Context.SaveChangesAsync(TestContext.Current.CancellationToken);

        var query = _fixture.Context.Set<Example>().Where(x => x.Name == $"{baseName}_X");
